			...
			if(pattern.matcher(w.cap.text).matches()) {
                ...
                for(Inventory inv: inventories) {
                        ...
                        for(WItem item: items) {
                                ...
                                Coord prevLocation = api.getItemLocation(item);                                                                
                                item.take();                                
                                api.drop(inv, x, y);  
                                api.msg(api.getItemName(item) + " place to " + Integer.toString(x) + "," + Integer.toString(y));  
                                api.drop(inv, prevLocation.x, prevLocation.y);                                        
                                api.msg(api.getItemName(item) + " back to " + Integer.toString(x) + "," + Integer.toString(y));  
                                // if (api.getItemAtHand() != null) {
                                //         api.msg(api.getItemName(api.getItemAtHand()) + " move to " + Integer.toString(x) + "," + Integer.toString(y));  
                                //         api.drop(inv, prevLocation.x, prevLocation.y);                                        
                                // }                            
